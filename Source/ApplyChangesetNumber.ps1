...
if($files)
{
	...
	foreach ($file in $files) {
		...
		if(-not $Disable)
		{
			...
			$VersionData = [regex]::matches($filecontent,$VersionRegex)
			$version = $VersionData[0].Value
			$versionWithoutChangeset = $version.Substring(0, $version.LastIndexOf(".") + 1);

			#change version to contain the changeset
			$newVersion = $versionWithoutChangeset + $Env:TF_BUILD_SOURCEGETVERSION.Substring(1, $Env:TF_BUILD_SOURCEGETVERSION.Length - 1)
